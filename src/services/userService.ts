...
export const registerUser = async (discordId: string): Promise<boolean> => {
	...
	let { body: character } = await supabase
	...
	await supabase.from<definitions['character_attributes']>('character_attributes').insert([
		{
...
export const getCharacterCurrentLocation = async (character: definitions['characters']): Promise<definitions['locations']> => {
	let { body: locations } = await supabase.from<definitions['locations']>('locations').select().eq('id', character.location_id);

	return locations![0];
};

export const checkCharacterExists = async (discordId: string) => {
	const res = await supabase.from<definitions['characters']>('characters').select('discord_id').eq('discord_id', discordId);
	if (res.body![0]) {
		return true;
	}
	return false;

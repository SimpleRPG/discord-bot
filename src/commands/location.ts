...
@ApplyOptions<SubCommandPluginCommandOptions>({
	...
	aliases: ['loc'],
	subCommands: ['move', { input: 'all', default: true }, { input: 'mv', output: 'move' }, { input: 'go', output: 'move' }]
})
export class UserCommand extends SubCommandPluginCommand {
	public async move(message: Message, args: Args) {
		...
		if (character.level < location.level) {
			return message.reply('You are not strong enough to enter that location!');
		}
